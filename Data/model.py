...
chatterbox['location_code'].replace(to_replace=408, value=0, inplace=True)
chatterbox['location_code'].replace(to_replace=415,  value=1, inplace=True)
chatterbox['location_code'].replace(to_replace=510,  value=2, inplace=True)
...
chatterbox = chatterbox.drop(columns=['State', 'voice_mail_plan', 'total_day_charge', 'total_eve_charge', 'total_night_charge', 'total_intl_charge'])
...
cat_model, catboost_accuracy = catboost_classifier(X, y)
lgbm_model, lgbm_accuracy = lgbm_classifier(X, y)

print(rf_accuracy, catboost_accuracy, lgbm_accuracy)

filename = 'Data/model_rf.sav'
joblib.dump(rf_model, filename)

filename = 'Data/model_cat.sav'
joblib.dump(cat_model, filename)

filename = 'Data/model_lgbm.sav'
joblib.dump(lgbm_model, filename)

# print(X.columns)
# print(y.columns)

X.to_csv('Data/model_data.csv' , index = False)
chatterbox.to_csv('Data/chatterbox.csv' , index = False)
